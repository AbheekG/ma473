function q1
	...
	H = BTCS(T, K, r, sig, R, Time, h, k, isTerminal);
	figure; plot(R(indices), H(1, indices)); hold on; plot(R(indices), H(end, indices)); hold off;
	legend('H(R,0) at t = 0', 'H(R,T) at t = T'); xlabel('R'); ylabel('H(R, t)'); title('H(R,t) using BTCS');
	saveas(gcf, sprintf('plots/q1_%d.png', im_num)); im_num = im_num + 1;
	figure; surf(R, Time, H); xlabel('R'); ylabel('t'); zlabel('H(R,t)'); title('H(R,t) using BTCS');
	saveas(gcf, sprintf('plots/q1_%d.png', im_num)); im_num = im_num + 1;
