function q1
	...
	% Greeks
	Do = Call_delta(T, K, r, sig, delta, S(2:end-1), 0);
	Go = Call_gamma(T, K, r, sig, delta, S(2:end-1), 0);
...
function [y] = Call_value(T, K, r, sig, delta, s, t)
	d1 = (1/(sig*sqrt(T-t))) * (log(s/K) + (r - delta + sig^2/2)*(T-t));
	d2 = (1/(sig*sqrt(T-t))) * (log(s/K) + (r - delta - sig^2/2)*(T-t));

	y = s*exp(-delta*(T-t))*normcdf(d1) - K*exp(-r*(T-t))*normcdf(d2);
	% if isnan(y)
	% 	T, K, r, sig, delta, s, t, d1, d2
	% end
end

function [y] = Call_delta(T, K, r, sig, delta, s, t)
	d1 = (1/(sig*sqrt(T-t))) * (log(s/K) + (r - delta + sig^2/2)*(T-t));
	y = exp(-delta*(T-t))*normcdf(d1);
end

function [y] = Call_gamma(T, K, r, sig, delta, s, t)
	d1 = (1/(sig*sqrt(T-t))) * (log(s/K) + (r - delta + sig^2/2)*(T-t));
...
function [U] = BS(T, K, r, sig, delta, S, Time, h, k)
	...
	for i = 1:n-1
		for j = 1:m
			U(i, j) = Call_value(T, K, r, sig, delta, S(j), Time(i));
		end
	end
