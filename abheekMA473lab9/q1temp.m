function q1
	...
	% For the basis/row of A
		...
		% For the Uj
			...
			% For integration interval
				A(i+1,j+1) = A(i+1,j+1) + (h/2) ...
					* (dphi2(x_min, x_max, N, i, X(k+1), 1)*dphi2(x_min, x_max, N, j, X(k+1), 1) ...
					+ dphi2(x_min, x_max, N, i, X(k+2), 0)*dphi2(x_min, x_max, N, j, X(k+2), 0));

				B(i+1,j+1) = B(i+1,j+1) + (h/2) ...
					* (bb(X(k+1))*phi2(x_min, x_max, N, i, X(k+1))*dphi2(x_min, x_max, N, j, X(k+1), 1) ...
					+ bb(X(k+2))*phi2(x_min, x_max, N, i, X(k+2))*dphi2(x_min, x_max, N, j, X(k+2), 0));

				C(i+1,j+1) = C(i+1,j+1) + (h/2) ...
					* (cc(X(k+1))*phi2(x_min, x_max, N, i, X(k+1))*phi2(x_min, x_max, N, j, X(k+1)) ...
					+ cc(X(k+2))*phi2(x_min, x_max, N, i, X(k+2))*phi2(x_min, x_max, N, j, X(k+2)));

				if j == 0
					d(i+1) = d(i+1) + (h/2) * (dd(X(k+1))*phi2(x_min, x_max, N, i, X(k+1)) ...
						+ dd(X(k+2))*phi2(x_min, x_max, N, i, X(k+2)));
				end
			end
		end
	end
	...
	% For the basis/row of A
		...
		% For the Uj
			...
			% For integration interval
				A(i+1,j+1) = A(i+1,j+1) + (h/6) ...
					* (dphi2(x_min, x_max, N, i, X(k+1), 1)*dphi2(x_min, x_max, N, j, X(k+1), 1) ...
					+ dphi2(x_min, x_max, N, i, (X(k+1)+X(k+2))/2, 1)*dphi2(x_min, x_max, N, j, (X(k+1)+X(k+2))/2, 1) ...
					+ dphi2(x_min, x_max, N, i, X(k+2), 0)*dphi2(x_min, x_max, N, j, X(k+2), 0));

				B(i+1,j+1) = B(i+1,j+1) + (h/6) ...
					* (bb(X(k+1))*phi2(x_min, x_max, N, i, X(k+1))*dphi2(x_min, x_max, N, j, X(k+1), 1) ...
					+ bb((X(k+1)+X(k+2))/2)*phi2(x_min, x_max, N, i,(X(k+1)+X(k+2))/2)*dphi2(x_min, x_max, N, j, (X(k+1)+X(k+2))/2, 1) ...
					+ bb(X(k+2))*phi2(x_min, x_max, N, i, X(k+2))*dphi2(x_min, x_max, N, j, X(k+2), 0));

				C(i+1,j+1) = C(i+1,j+1) + (h/6) ...
					* (cc(X(k+1))*phi2(x_min, x_max, N, i, X(k+1))*phi2(x_min, x_max, N, j, X(k+1)) ...
					+ cc((X(k+1)+X(k+2))/2)*phi2(x_min, x_max, N, i, (X(k+1)+X(k+2))/2)*phi2(x_min, x_max, N, j, (X(k+1)+X(k+2))/2) ...
					+ cc(X(k+2))*phi2(x_min, x_max, N, i, X(k+2))*phi2(x_min, x_max, N, j, X(k+2)));

				if j == 0
					d(i+1) = d(i+1) + (h/6) * (dd(X(k+1))*phi2(x_min, x_max, N, i, X(k+1)) ...
						+ dd((X(k+1)+X(k+2))/2)*phi2(x_min, x_max, N, i, (X(k+1)+X(k+2))/2) ...
						+ dd(X(k+2))*phi2(x_min, x_max, N, i, X(k+2)));
				end
			end
		end
